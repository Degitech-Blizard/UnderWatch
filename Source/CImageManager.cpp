...
void CImageManager::Begin(RenderMode _Mode)
{
	//m_Line->Begin();

	switch (_Mode)
	{
	case RenderMode::RM_Normal:
		m_pSprite->Begin(D3DXSPRITE_ALPHABLEND | D3DXSPRITE_OBJECTSPACE);
		break;
	case RenderMode::RM_UI:
		m_pSprite->Begin(D3DXSPRITE_ALPHABLEND);
		break;
	case RenderMode::RM_Billboard:
		m_pSprite->SetWorldViewLH(NULL, &CAMERA.m_matView);
		m_pSprite->Begin(D3DXSPRITE_BILLBOARD | D3DXSPRITE_ALPHABLEND | D3DXSPRITE_OBJECTSPACE);
		break;
	}
...
void CImageManager::ReBegin(RenderMode _Mode)
{
	//m_Line->End();
	m_pSprite->End();
	//m_Line->Begin();
...
void CImageManager::Render(CTexture * _pTexture, CRVector2 _vPos, CRVector2 _vScale,FLOAT _fDegree, CRRect _reRect, Color _Color)
{
	...
	m_pSprite->Draw(_pTexture->m_pTexture, &(RECT)_reRect, &vCenter, nullptr, D3DXCOLOR(0,0,0,0));
...
void CImageManager::Render(CTexture * _pTexture, CRMatrix _WorldMat, CRRect _reRect, Color _Color)
{
	...
	m_pSprite->SetTransform(&a);

	m_pSprite->Draw(_pTexture->m_pTexture, &(RECT)_reRect, &vCenter, nullptr, _Color);
}

void CImageManager::BillboardRender(CTexture * _pTexture, CRMatrix _WorldMat, CRRect _reRect, Color _Color)
{
	Vector3 vCenter = Vector3(
		_pTexture->m_Info.Width / 2.f,
		_pTexture->m_Info.Height / 2.f,
