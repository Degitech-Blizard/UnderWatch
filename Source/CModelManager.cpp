...
CModelManager::CModelManager()
{
	...
	this->AddShader("A", "ComputeShader.fx");
}
...
CModel * CModelManager::AddModel(CRString _Key, CRString _Path, CRString _MapPath)
{
	...
	if (find != m_mapModel.end())
	{
		...
		COBJLoader * a = new COBJLoader();
		a->OBJLoad(pModel, _Path, _MapPath);
		SAFE_DELETE(a);
	return pModel;
}
	...
	if (_pModel)
	{
		if (_pModel->m_pMesh)
		{
			...
			for (auto iter : _pModel->m_vecMaterial)
			{
				...
				else g_Device->SetTexture(0, nullptr);

				_pModel->m_pMesh->DrawSubset(i++);
			}

			//FX->SetMatrix((D3DXHANDLE)"WorldViewMatrix", &(_matWorld * CAMERA.m_matView));


			//FX->SetMatrix((D3DXHANDLE)"gWorldMatrix", &_matWorld);
			//FX->SetMatrix((D3DXHANDLE)"gViewMatrix", &CAMERA.m_matView);
			//FX->SetMatrix((D3DXHANDLE)"gProjMatrix", &CAMERA.m_matProj);

			//FX->SetTechnique((D3DXHANDLE)"Tech");
